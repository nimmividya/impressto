...
class admin_remote extends PSBase_Controller {
	...
	public function __construct(){
		...
		//if(!$this->db->table_exists('showcases')) $this->install();
	...
	*/
	public function list_gallery_categories($gallery){
		...
		$this->load->model('showcase_admin_model');
	
		$data['gallery_categories'] = $this->showcase_admin_model->get_gallery_categories($gallery);
	...
	*/
	public function list_gallery_items($gallery, $category = ''){
		...
		$data['galleryrecords'] = $this->showcase_admin_model->get_galleryitems($gallery, $category);
		
		$data['category_name'] = $this->showcase_admin_model->get_category_name($category);
		...
		'options' =>  $this->showcase_admin_model->get_category_selector(),
		'value'       => $category
		);
		...
		$sql = "SELECT category_image FROM {$this->db->dbprefix}showcase_categories WHERE id='{$category}'";
		...
		echo $this->load->view('showcase_list', $data, TRUE);
	...
	*/
	public function upload_image(){
		...
		$settings = $this->showcase_model->getGallerySettings($gallery);
	...
	}else 
	{
			...
			$image_path = ASSET_ROOT . "uploads/" . PROJECTNAME . "/showcase/{$gallery}/{$category}";
	...
	*/
	public function savecrop_dimensions(){
		...
		$widget_settings = $this->showcase_model->get_widget_settings();
		...
		$image_path = ASSET_ROOT . "uploads/" . PROJECTNAME . "/showcase/{$this->input->get('gallery')}/{$this->input->post('thumb_image_category')}";
	...
	*/	
	public function edit_category($id){
		...
		$sql = "SELECT * FROM {$this->db->dbprefix}showcase_categories WHERE id = '{$id}'";
		...
		if ($query->num_rows() > 0){
			...
			$sql = "SELECT imagename FROM {$this->db->dbprefix}showcase_items WHERE id = '{$row->category_image}'";
	...
	*/	
	public function edit_item($id){
		...
		$sql = "SELECT * FROM {$this->db->dbprefix}showcase_items WHERE id = '{$id}'";
		...
		if ($query->num_rows() > 0){
			...
			$sql = "SELECT category_image FROM {$this->db->dbprefix}showcase_categories WHERE id = '{$row->category}'";
	...
	*/
	public function deletecategory($id){
		...
		$this->db->delete('showcase_categories', array('id' => $id)); 
	...
	public function deleteitem($id){
		...
		$sql = "SELECT category, imagename FROM {$this->db->dbprefix}showcase_items WHERE id = '{$id}'";
		...
		if ($query->num_rows() > 0){
			...
			$image_path = ASSET_ROOT . "uploads/" . PROJECTNAME . "/showcase/{$gallery}/{$category_id}";
			$thumbnail_path = $image_path . "/thumbs";
			$original_path = $image_path . "/originals";
			...
			$sql = "DELETE FROM {$this->db->dbprefix}showcase_items WHERE id = '{$id}'";
	...
	*/
	public function saveimagedetails(){
		...
		if($id == ""){
			...
			$this->db->insert('showcase_items', $data); 
			...
			$query = $this->db->get_where('showcase_items', array('id' => $id));
			...
			if ($query->num_rows() > 0)
			{
				...
				if($old_category != $category){
					...
					$gallerypath = ASSET_ROOT . "uploads/" . PROJECTNAME . "/showcase/{$gallery}";
			...
			$this->db->where('id', $id);
			$this->db->update('showcase_items', $data); 
		...
		if($is_default_category_image){
			...
			$this->db->update('showcase_categories', $data); 
	...
	*/
	public function savenewgallery(){
		...
		$this->widget_utils->register_widget("showcase","showcase",$olddata['name'], $data['name']);
	...
	*/
	public function deleteGallery($id){
		...
		//$sql = "SELECT * FROM {$this->db->dbprefix('showcases')} WHERE id = '{$id}'"; 
		
		$query = $this->db->get_where('showcases', array('id' => $id));
		...
		if ($query->num_rows() > 0)
		{
			...
			$this->db->delete('showcases', array('id' => $id)); 
			$this->db->delete('showcase_items', array('gallery' => $id)); 
			$this->db->delete('showcase_categories', array('gallery' => $id)); 
			
			$this->widget_utils->un_register_widget("showcase","showcase",$row->name);
	...
	*/
	public function saveGallery(){
		...
		if($this->widget_utils->getwidgetid("showcase","showcase",$data['name'])){
			...
			$this->db->insert('showcase_categories', $catdata);
	

			// register it to the system
			$this->widget_utils->register_widget("showcase","showcase",$data['name']);
			
			//$return_array['gallerylist'] = $this->showcase_admin_model->get_galleries();
	...
	public function galleryList(){
		...
		echo json_encode($this->showcase_admin_model->get_galleries());
	...
	*/
	...
	public function savenewcategory(){
		...
		$sql = "SELECT MAX(position) AS maxpos FROM {$this->db->dbprefix}showcase_categories WHERE gallery = '{$gallery}'";
		$query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM {$this->db->dbprefix}showcase_categories WHERE name = '{$new_category}' AND gallery = '{$gallery}'";
		$query = $this->db->query($sql);
	
		if ($query->num_rows() == 0) {

			$sql = "INSERT INTO {$this->db->dbprefix}showcase_categories (name,gallery,position) VALUES (";
			$sql .= "'{$new_category}',";
			$sql .= "'{$gallery}',";
			$sql .= "'{$maxpos}'";
			$sql .= ")";
	...
	*/	
	public function reposition_categories(){
		...
		foreach($ids AS $id){
			...
			$sql = "UPDATE {$this->db->dbprefix}showcase_categories SET position = '{$i}' WHERE id='{$id}'";
			$this->db->query($sql);
	...
	*/	
	public function move_category_position(){
		...
		$sql = "SELECT id FROM {$this->db->dbprefix}showcase_categories ORDER BY position";
		...
		foreach ($query->result() as $row)
		{
			...
			$sql = "UPDATE {$this->db->dbprefix}showcase_categories SET position = '{$position}' WHERE  id = '{$row->id}'";
			$this->db->query($sql);
		...
		if($direction == "up"){
			...
			$sql = "UPDATE {$this->db->dbprefix}showcase_categories SET position = (position -1) WHERE  id = '" . $positions_array['item_' . $target_position] . "'";
			$this->db->query($sql);
			$sql = "UPDATE {$this->db->dbprefix}showcase_categories SET position = (position +1) WHERE  id = '" . $positions_array['item_' . ($target_position - 1)] . "'";
			$this->db->query($sql);	
		}
	...
	*/	
	public function reposition_images(){
		...
		foreach($ids AS $id){
			...
			$sql = "UPDATE {$this->db->dbprefix('showcase_items')} SET position = '{$i}' WHERE id='{$id}'";
			$this->db->query($sql);
	...
	*/	
	public function move_position(){
		...
		// fill in all the gaps first.
		$sql = "SELECT id FROM {$this->db->dbprefix}showcase_items WHERE gallery = '{$gallery}' AND category = '{$category}' ORDER BY position";
		$query = $this->db->query($sql);
		...
		foreach ($query->result() as $row)
		{
			...
			$sql = "UPDATE {$this->db->dbprefix}showcase_items SET position = '{$position}' WHERE  id = '{$row->id}'";
			$this->db->query($sql);
		...
		if($direction == "up"){
			...
			$sql = "UPDATE {$this->db->dbprefix}showcase_items SET position = (position -1) WHERE  id = '" . $positions_array['item_' . $target_position] . "'";
			$this->db->query($sql);
			$sql = "UPDATE {$this->db->dbprefix}showcase_items SET position = (position +1) WHERE  id = '" . $positions_array['item_' . ($target_position - 1)] . "'";
			$this->db->query($sql);	
		}
	...
	*/
	public function getgallerysettings($gallery){
		...
		// set defaults
		echo json_encode($this->showcase_model->getGallerySettings($gallery));
	...
	*/
	public function savegallerysettings($gallery){
		...
		$this->db->delete('showcase_settings', array('gallery' => $gallery)); 
		...
		$this->db->insert('showcase_settings', $insertdata); 
	...
	private function init_impressto(){
		...
		//$this->load->library('impressto');
		//$this->impressto->setDir(APPPATH . "/custom_modules/showcase/views/ps_templates/");
