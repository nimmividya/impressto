...
class showcase extends PSAdmin_Controller {
	...
	public function __construct(){
		...
		if(!$this->db->table_exists($this->db->dbprefix .'showcases')){
	...
	*/
	...
	private function init_impressto(){
		...
		$this->impressto->setDir(APPPATH . "/custom_modules/showcase/widgets/views/{$docketdir}/impressto/");
	...
	*/
	public function index($param = ''){
		...
		$this->asset_loader->add_header_js(ASSETURL . PROJECTNAME . "/custom_modules/showcase/js/showcase_manager.js");
		...
		$this->asset_loader->add_header_js(ASSETURL . PROJECTNAME  . "/default/vendor/bootbox.min.js");
		...
		$data['template_options'] = array_merge(array("Select"=>""),$this->template_loader->find_templates(array("module"=>"showcase","is_widget"=>TRUE)));
		...
		//$data['galleryrecords'] = $this->showcase_admin_model->get_galleryitems($category);
		...
		$data['showcases'] = $this->showcase_admin_model->get_galleries();
		...
		*/
		...
		$data['gallery_list'] = $this->load->view('showcase_list', $data, TRUE);
		
							
		//$data['gallery_categories'] = $this->showcase_admin_model->get_gallery_categories();
		...
		$data['main_content'] = 'showcase_manager';
	...
	*/
	public function install(){
		...
		$this->widget_utils->register_widget("showcase","showcase");
	...
	*/
	public function uninstall(){
		...
		$this->widget_utils->un_register_widget("showcase","showcase");
...
function uploadimage(){
	...
	}else 
	{
			...
			$temp_image_path = ASSET_ROOT . "uploads/".PROJECTNAME."/showcase/temp/" . $_FILES[$fileElementName]["name"];
			
			move_uploaded_file($_FILES[$fileElementName]["tmp_name"],$temp_image_path);
			
					
			if(!file_exists(ASSET_ROOT . "uploads/".PROJECTNAME."/showcase/" . $_POST['widget_name'])){
			
				mkdir(ASSET_ROOT . "uploads/".PROJECTNAME."/showcase/" . $_POST['widget_name']);
...
function save(){
	...
	$temp_image_path = ASSET_ROOT . "uploads/".PROJECTNAME."/showcase/temp/" . $this->input->post('temp_image');
				
	$final_image_path = ASSET_ROOT . "uploads/".PROJECTNAME."/showcase/" . $this->input->post('form_widget_name') . "/" . $this->input->post('temp_image');

	$thumbnail_folder = ASSET_ROOT . "uploads/".PROJECTNAME."/showcase/" . $this->input->post('form_widget_name') . "/thumbnails/";
...
public function moveitem_up($id){
	...
	$this->showcase_model->moveitem("up",$id);
}


public function moveitem_down($id){
	
	$this->showcase_model->moveitem("down",$id);
