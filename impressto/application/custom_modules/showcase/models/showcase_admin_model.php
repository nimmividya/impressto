...
class showcase_admin_model extends showcase_model{
	...
	*/
	public function get_galleries(){
		...
		$sql = "SELECT * FROM {$this->db->dbprefix('showcases')} ORDER BY name ASC ";
	...
	*/
	...
	public function get_gallery_categories($gallery){
		...
		$query = $this->db->get('showcase_categories');
		...
		if ($query->num_rows() > 0){
			...
			foreach ($query->result() as $row){
				...
				$num_images = $this->db->get('showcase_items')->num_rows();
