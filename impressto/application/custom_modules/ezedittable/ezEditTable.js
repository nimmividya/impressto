/*------------------------------------------------------------------------
	- ezEditTable v2.1 (edittable.free.fr)
...
========================================================================*/
	...
	*********************************************/
	...
	this.editorAllowEmptyValue = [],this.editorCmdColIndex, this.editorCmdBtns = {}, this.editorUploader = [];
	...
	this.updateSubmitMethod = this.updateConfig.hasOwnProperty('submit_method') ? this.updateConfig['submit_method'].LCase() : 'form';
	this.insertSubmitMethod = this.insertConfig.hasOwnProperty('submit_method') ? this.insertConfig['submit_method'].LCase() : 'form';
	this.deleteSubmitMethod = this.deleteConfig.hasOwnProperty('submit_method') ? this.deleteConfig['submit_method'].LCase() : 'form';
	...
	this.msgNewRowNoUploader = f.msg_new_row_no_uploader!=undefined ? f.msg_new_row_no_uploader : 'Please save the added rows before using the Uploader.';
...
Uploader.prototype = {
	...
	SetUploader: function(){
		...
		this.divUplInfo.innerHTML = parseInt(this.o.uplMaxFileSize[this.colIndex]/1024) + 'Kb max (' + this.o.uplValidExt[this.colIndex].toString() + ')';
...
Editable.prototype = {
    ...
    CreateInputEditor: function(colIndex){
        ...
        this.o.Event.Add(inp, 'keypress', function(e){ x.Event.OnKeyPress.call(x, e); });
        return inp;
    },
    ...
    SetCommandEditor: function(colIndex){
        ...
        for (var i = this.o.startRow; i < this.o.GetRowsNb(); i++){
            ...
            if(this.o.cmdEnabledBtns.indexOf('cancel') != -1){
                ...
                this.o.Event.Add(cancelBtn, 'click', function(e){ x.CloseRowEditor(); });
    ...
    AddNewRow: function(){
        var row = this.o.table.insertRow(this.o.startRow);
    ...
    Submit: function(cmd){
        ...
        if(onSubmit) onSubmit.call(null, this.o, treatedRows);
            ...
            if(!uri){ alert(this.o.msgUndefinedSubmitUrl); }
            else {
                        ...
                        try {//window.open(uri+prm);
                            this.o.IncludeFile(this.o.prfxScr + rowIndex + '_' + this.o.id, uri + prm,
								function(eg, scriptElm){
								    ...
								    eg.Editable.RemoveModifiedCellMark(parseInt(rIndex)); 
									if(eg.savedRowsNb[cmd] === treatedRows.length){ SubmitComplete(); }
								});
                        } catch (e){
								...
								try{
									...
									if(iframeDoc && iframeDoc.readyState === 'complete'){
										...
										if(iframeDoc.location.href === 'about:blank'){
											...
											if(x.o.savedRowsNb[cmd] === treatedRows.length){ SubmitComplete(); }
										}
									}
								} catch(e) {
        ...
        function SubmitComplete(){
            if(x.o.savedRowsNb[cmd] === treatedRows.length){
                ...
                if(cmd == 'insert'){ x.o.Editable.newRows = []; x.o.Editable.addedRows = []; }
                    ...
                    var a = [];
                    for (var k = 0; k < x.o.Editable.deletedRows.length; k++) a.push(x.o.Editable.deletedRows[k][0]);
                    a.sort(x.o.Sort.NumDesc); //rows indexes need to be sorted in desc manner for rows deleting operation
                    for (var k = 0; k < a.length; k++){ x.o.table.deleteRow(a[k]); }
				...
				if(x.o.savedRowsNb.update === 0 && x.o.savedRowsNb.insert === 0 && x.o.savedRowsNb['delete'] === 0){
					if(afterSubmitCallBack){ afterSubmitCallBack.call(null, x.o, treatedRows); }
						...
						if(!x.o.autoSave && (!x.o.Editable.submitAll || (x.o.Editable.submitAll && cmd === 'update'))){ alert(x.o.msgSubmitOK); } 
					}
					x.o.Editable.submitAll = false;
				}				
                if(x.o.onServerActivityStop) x.o.onServerActivityStop.call(null, x.o, x.o.table.rows[rowIndex]);
                if(submitMethod === 'form') x.o.ifrmContainer[cmd].innerHTML = '';
				x.o.Css.Remove(x.o.table, x.o.activityIndicatorCss);
            }
        }
    },
    ...
    Event: {
        ...
        OnKeyPress: function(e){
            ...
            switch (keyCode){
                case 13: //enter
                    if(!this.o.selection && this.o.IsEditorType(colIndex, this.o.edtTypes.input) && this.o.editorModel == 'cell') this.CloseCellEditor(colIndex);
                    break;
                case 27: //escape
                    if(this.o.editorModel == 'cell'){ if(this.activeCellEditor) this.CloseCellEditor(colIndex); }
                    if(this.o.editorModel == 'row') this.CloseRowEditor();
                    break;
                case 9: //tab
                    if(!this.o.selection && this.o.editorModel == 'cell') this.CloseCellEditor(colIndex);
                    break;
            }
        }
    }
