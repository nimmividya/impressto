...
// sample tags
//  [widget type='member_manager/member_manager']
//  direct from PHP code Widget::run('member_manager/member_manager', array('name'=>'widget_1'));

// within smarty {widget type='member_manager/member_manager' name='widget1'}



class member_manager extends Widget
{
    function run() {
		...
		$this->load->_add_module_paths('member_manager');
		$this->load->model('member_manager/member_manager_model');
		...
		if(!isset($widget_args['widget_id']) && isset($data['name'])) 
			$widget_args['widget_id'] = $this->widget_utils->getwidgetid('member_manager', 'member_manager', $data['name']);


		
			
		// load the news widget language file now...
		$this->lang->load('member_manager', '', FALSE, TRUE, dirname(dirname(__FILE__)) . '/');	
		...
		$moduleoptions = ps_getmoduleoptions('member_manager');
		...
		$data['module'] = 'member_manager';
		$data['is_widget'] = TRUE;
		$data['widgettype'] = 'member_manager';	
