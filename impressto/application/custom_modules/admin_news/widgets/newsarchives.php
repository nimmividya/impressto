...
class newsarchives extends Widget
{
	...
	*/
    function run() {
		...
		if(!isset($widget_args['widget_id']) && isset($data['name'])) 
			$widget_args['widget_id'] = $this->widget_utils->getwidgetid('newsarchives', 'admin_news', $data['name']);
		...
		$data['newspageurl'] = $this->remove_querystring_var($this->current_url(), 'news_pager');
		...
		//$moduleoptions['news_listlimit'] = 0;
		...
		if($this->input->get_post('news_id') != ""){
			...
			$newsrecords = $this->news_model->get_archived_items('', $data['news_tag'],$data['news_pager'], $moduleoptions['news_listlimit']);
			...
			$newsrecords = $this->news_model->get_archived_items($this->input->get_post('news_search_keywords'),'',$data['news_pager'], $moduleoptions['news_listlimit']);
			...
			// regular full list
			$newsrecords = $this->news_model->get_archived_items('','', $data['news_pager'], $moduleoptions['news_listlimit']);
	...
	/**
	* Basically just remove ? and & from the end of the url
	*
	*/	
