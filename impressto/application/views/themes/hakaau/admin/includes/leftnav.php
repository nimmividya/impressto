...
'users' => array('icon' =>'icon-user', 'menutrigger' => 'collapseFive'),
'config' => array('icon' =>'icon-wrench', 'menutrigger' => 'collapseSix'),
);
...
foreach($autosections AS $section => $sectionval){
	...
	echo "<i class=\"{$sectionval['icon']}\"></i> ";
		
	echo lang('leftnav_' . $section);



	echo "</a>";

	if($listitems != ""){  echo "<ul class=\"closed\" >{$listitems}</ul>";
