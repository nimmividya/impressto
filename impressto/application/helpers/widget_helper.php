...
/**
* Generic utility functions for using widgets in PHP or impressto templates
*
*/

function widgetzone($name)
{
	...
	$sql = "SELECT id FROM {$CI->db->dbprefix}widget_zones WHERE name = '{$name}'";
	...
	if ($query1->num_rows() > 0){
		...
		if(isset($page_id)){
			...
			// Nimmitha Vidyathilaka - a somewhat pointless query but saves me from rewriting the whole function
