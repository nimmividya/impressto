...
class file_tools{
	...
	*/
	...
	///////////////////////////////////////////
	//
	function Upload($fieldname, $files_dir, $disallowed_extensions, $allowed_extensions){


		$returnarray = array();

		$iCount = count( $_FILES[$fieldname]['name'] );
		...
		for( $i = 0; $i < $iCount; $i++ ){
			...
			if( !empty( $_FILES[$fieldname]['name'][$i] ) ){
				...
				$aFile['tmp_name'] =  $_FILES[$fieldname]['tmp_name'][$i];
				$aFile['name'] =      $_FILES[$fieldname]['name'][$i];

				$fExtension = $this->Get_Extension($aFile['name']);

				$continue = true;

				if(is_array($disallowed_extensions) && in_array($fExtension,$disallowed_extensions)) $continue = false;
				if(is_array($allowed_extensions) && !in_array($fExtension,$allowed_extensions)) $continue = false;
				...
				if($continue){
					...
					while( file_exists ($files_dir . "/".$filename)) {
						$filename = preg_replace ( "/^(.+?)(_?)(\d*)(\.[^.]+)?$/e", "'\$1_'.(\$3+1).'\$4'", $filename );
					}

					$target = $files_dir . "/".$filename;

					if(move_uploaded_file ( $aFile['tmp_name'],$target)){
