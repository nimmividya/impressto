var psimagemanager = appbase.extend({
...
construct: function() {
	...
	toggle_maintain_aspect : function(obj){
		
		if(obj.checked){
			
			this.init_resize_slider(100);
	...
	change_dimensions: function(jqobj){
	
		if( !this._isNumber(jqobj.val()) ) return;
		
		
		if ($('#maintain_aspect').is(':checked')) {
		
			var original_width = $('#wR').val();
			var original_height = $('#hR').val();
			var scalefactor = 0;
	
			if(jqobj.attr('id') == "resizeImageWidth"){
			
				scalefactor = (jqobj.val() / original_width);
				$('#resizeImageHeight').val( Math.round( original_height * scalefactor ) );
			}else{
				scalefactor = (jqobj.val() / original_height);
			
				$('#resizeImageWidth').val( Math.round( original_width * scalefactor ) );
			}
			
			var percent = Math.round(scalefactor * 100);
						
			$("#resize_slider").slider('value', percent);
			$('#percentage_resize_indicator').html(percent + "%"); 
	...
	init_resize_slider : function(value){
		

		var original_width = $('#wR').val();
		var original_height = $('#hR').val();
		
		var new_width = $('#wR').val();
		var new_height = $('#hR').val();
		
		var scale = 1;
		
		
		$("#resize_slider").slider({ 

max: 300, // allow up to 5 times increase
value: value,
slide: function(event, ui) {
				
				scale = (ui.value / 100);
				
				new_width = Math.round(scale * original_width);
				new_height = Math.round(scale * original_height);
				
				$('#resizeImageWidth').val(new_width);
				$('#resizeImageHeight').val(new_height);
				
				$('#resize_slider_wrapper').fadeIn();
				
				$('#percentage_resize_indicator').html(ui.value + "%"); 
...
var psimgmgr = new psimagemanager();
...
var sUrl = '/image_manager/crop_image';
var asset_url = '';
...
success: function(data) {
	...
	$('.folderOpened,.folderS').live('click', function() {
...
marginTop: '-' + Math.round(scaley * coords.y) + 'px'
	...
	$('#resizeButton').live('click', function() {
		...
		psimgmgr.init_resize_slider(100); // 100 % is normal scaling
...
success: function(data) {
	...
	$(function() {
		...
		$('#resizeImageWidth, #resizeImageHeight').change(function() {
			psimgmgr.change_dimensions($(this));		
		});
		...
		$('.addimages').click(function() {
			...
			// let's save clicked element id to remember it while we'll select images in popup:

			$('#clickedElementId').val('#' + this.id + 'div');

			// remove old images from selected previously in popup selector:
			$("#selectedImages .selectedImgBlockMain").remove();

			// if there's any images already has been selected to test div, make cycle on them and get them all to popup selector to be able to re-order/remove/add:

			if($($("#clickedElementId").val() + ' .democontent').children('div').size()>0) {
				$($("#clickedElementId").val() + ' .democontent').children('div').each(function() {
					linkto = $(this).attr("linkto");
					filename = $(this).attr("filename");
					filePath = $(this).attr("filePath");
					fileThumb = filePath + "/thumbs/" + filename;
					fname = $(this).attr("fname");
					fileExt = $(this).attr("ext");
					date = $(this).attr("date");

					// appending images from test div to popup selector 'selected' area:

					$("#selectedImages").append('<div class="ui-draggable selectedImgBlockMain" id="' + linkto + '_clone" filename="' + filename + '" fname="' + fname + '" ext="' + fileExt + '" path="' + filePath + '" linkto="' + linkto + '" date="' + date + '"><div class="imageName selectedImgBlock">' + fname + '</div><div class="imageImage" style="height: 36px; width: 36px; left: 0px; "><img src="' + fileThumb + '" alt="01" style="width: 77px !important;"></div></div>');
				});

				// make 'selected' area sortable:

				$("#selectedImages").sortable();
			}

			// call main div with the image manager and show it in modal dialog:
...
top: '270px',
	...
	pasteSelectedImages = function(link) {
		var clickedid = $('#clickedElementId').val();
		$($("#clickedElementId").val() + ' .democontent').html('');

		// here we click 'Done' button on dialog and returning all selected images to somewhere:

		$("#selectedImages").children('div.selectedImgBlockMain').each(function() {

			linkto = $(this).attr("linkto"); // example: linkto="/storage/images/NewFolderName/111205a9686.jpg?rand=7445" 
			filePath = $(this).attr("path"); // example: filepath="/storage/images/NewFolderName" 
			fname = $(this).attr("fname"); // example: fname="111205a9686" 
			ext = $(this).attr("ext"); // example: ext="JPG" 
			date = $(this).attr("date"); // example: date="09.08.2012 12:19
			filename = $(this).attr("filename"); // example: filename="111205a9686.jpg" 
			fileThumb = filePath + "/thumbs/" + filename; // example: filethumb="/storage/images/NewFolderName/thumbs/111205a9686.jpg" 


			// if needed to get different structure of output, just change it here inside of 'append' function

			$($("#clickedElementId").val() + ' .democontent').append('<div class="divTestImg" linkto="' + linkto + '" fileName="' + filename + '" filePath="' + filePath + '" fileThumb="' + fileThumb + '" fname="' + fname + '" ext="' + ext + '" date="' + date + '"><img src="' + fileThumb + '"></div>');

			// as result we get code like this:
			// <div class="divTestImg" linkto="/storage/images/NewFolderName/111205a9686.jpg?rand=7445" filename="111205a9686.jpg" filepath="/storage/images/NewFolderName" filethumb="/storage/images/NewFolderName/thumbs/111205a9686.jpg" fname="111205a9686" ext="JPG" date="09.08.2012 12:19"><img src="/storage/images/NewFolderName/thumbs/111205a9686.jpg"></div>
