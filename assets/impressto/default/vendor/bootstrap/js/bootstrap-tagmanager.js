...
(function (jQuery) {
   ...
   jQuery.fn.spliceTag = function (parentfield,TagId,robj) { // Nimmitha Vidyathilaka localization
      var obj;
      if (robj == null)
         return;
      else
	 ...
	 if (-1 != p) {
         ...
         jQuery("#" + parentfield + "_myTag_" + TagId).remove();
         tlis.splice(p, 1);
         tlid.splice(p, 1);
   ...
   jQuery.fn.pushTag = function (tag, robj) {
      ...
      if (alreadyInList) {
         ...
         html += '<span class="myTag" id="' + tagManagerOptions.taggerfieldname + '_myTag_' + TagId + '"><span>' + tag + '&nbsp;&nbsp;</span><a href="#" class="myTagRemover" id="' + tagManagerOptions.taggerfieldname + '_myRemover_' + TagId + '" parentfield="' + tagManagerOptions.taggerfieldname + '" TagIdToRemove="' + TagId + '" title="Removing tag">x</a></span>';
      
		 obj.before(html);
		 
		// jQuery("#myRemover_" + TagId).on("click", obj, function (e) { 
    	 jQuery("#" + tagManagerOptions.taggerfieldname + "_myRemover_" + TagId).on("click", obj, function (e) { // localized by Nimmitha Vidyathilaka
				
			e.preventDefault();
            var TagIdToRemove = parseInt(jQuery(this).attr("TagIdToRemove"));
			var parentfield = jQuery(this).attr("parentfield");

            e.data.spliceTag(parentfield,TagIdToRemove,e.data);
   ...
   jQuery.fn.tagsManager = function (options) {
      tagManagerOptions = {
		 ...
		 taggerfieldname: 'hiddenTagList', // localized by Nimmitha Vidyathilaka
      ...
      return this.each(function() {
         ...
         //html += "<input name='hiddenTagList' type='hidden' value=''/>";
		  html += "<input name='" + tagManagerOptions.taggerfieldname + "' type='hidden' value=''/>"; // localized by Nimmitha Vidyathilaka
         ...
         obj.data("lhiddenTagList", 
			...
			obj.siblings("input[name='" + tagManagerOptions.taggerfieldname + "']")[0] // localized by Nimmitha Vidyathilaka
